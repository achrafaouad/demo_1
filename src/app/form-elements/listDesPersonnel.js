...
class PopupPers extends React.Component {
  constructor(props){
    super(props);
    this.state = {travaux:[]};
  ...
  handleCh(e){
      ...
      console.log("zamel" , e.target.value )
      if(isChecked){
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevState.travaux !== this.state.travaux) {
      ...
      this.props.ListPers(this.state.travaux)
    }
  ...
  parc(){
      if(this.state.data){
        ...
        return this.state.data.map((st,index)=>{
            ...
            <input class="form-check-input" type="checkbox" value={[st.nom, st.salaire_hr,st.id_pers]} id={"test"+index + st.nom } onChange={this.handleCh}/>
